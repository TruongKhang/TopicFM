...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=1,
                 depths=[1, 1, 1, 3], dims=[96, 192, 256, 384], drop_path_rate=0., 
                 layer_scale_init_value=1e-6, head_init_scale=1.,
                 ):
        super().__init__()

        self.model_size = len(depths)
        self.downsample_layers = nn.ModuleList() # stem and 3 intermediate downsampling conv layers
        stem = nn.Sequential(
            nn.Conv2d(in_chans, dims[0], kernel_size=2, stride=2, padding=0),
    ...
    def forward_features(self, x):
        ...
        for i in range(1, self.model_size):
            x_out = self.downsample_layers[i](x_out)
            x_out = self.stages[i](x_out)
        return (x_out, x0) # global average pooling, (N, C, H, W) -> (N, C)
